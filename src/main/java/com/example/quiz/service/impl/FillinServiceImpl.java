...
public class FillinServiceImpl implements FillinService {
		...
		try {
			...
			for (Question item : quList) {
				...
				for (Fillin fillin : fillinList) {
					...
					// 如果qIdList已經有包含問題編號，就表示已檢查過該題號
					// 此段用來排除req中有重複的問題編號
					if (qIdList.contains(fillin.getqId())) {
						continue;
					}
					...
					// 新增相同題號的fillin
					// 不直接把fillin放入list是因為:
					// 上面的程式碼只有對question_id和answer做檢查，所以其餘的屬性內容可能是不合法的
					// 直接使用Question item的值是因為這些值都是從DB來，當初已有檢查過
				...
				// 檢查必填
				if (item.isNecessary() && !qIdList.contains(item.getId())) {
	...
	private BasicRes checkOptionAnswer(Question item, Fillin fillin) {
		...
		// 3. 排除簡答題；option type 是 text(因為接下來要檢查答案跟選項是否一致)
			...
			try {
				...
				for (Fillin fillin : fillinList) {
					...
					for (String option : optionArray) {
						// 比對答案中每個選項出現的次數
						// 避免某個選項是另一個選項的其中一部份
						// 例如: 綠茶;綠茶拿鐵;梅子綠茶 都是選項，要找出綠茶出現次數，綠茶苶鐵和梅子綠茶都不能算
						// 所以需要在每個選項前後再加上分號，會用分號是因為答案的串接使用分號
						// 後續要找出現次數時就會使用 ;綠茶;來找
						String newOption = ";" + option + ";";
						// 透過將選項被空白取代，這樣可以計算出減少的長度
						String newAnswer = answer.replace(newOption, "");
						// 計算該選項出現的次數
						// 原本字串長度 - 被取代後字串的長度，要除以選項的長度才會是真正的次數
						int count = (answer.length() - newAnswer.length()) / newOption.length();
						// 記錄每一題的統計
						optionCountMap = countMap.getOrDefault(fillin.getqId(), optionCountMap);
						// 先取出選項(key)對應的次數
						// getOrDefault(option, 0): map 中沒有 option(key) 的話，會返回 0
						...
						// Statistics statistics = new Statistics(fillin.getqId(),
						// fillin.getQuestiion(),//
